...
extern void RIL_requestTimedCallback (int timeReq,
...
static struct RIL_Env s_rilEnv_inst0 = {
    RIL_onRequestComplete,						/* 此处是函数，在ril.cpp中定义 */
...
int main(int argc, char **argv)
{
    ...
    LOGD("**RIL Daemon Started**");
    LOGD("**RILd param count=%d**", argc);
    for (i = 0; i < argc; i++) {
        LOGD("**RIL Daemon argv[%d] = %s**", i, argv[i]);
    }
...
#define REFERENCE_RIL_PATH "/system/lib/libril-at-cyit.so"
...
OpenLib:
    ...
    LOGD("RIL_Init argc = %d client = %s",argc, s_argv[argc-1]);
    LOGD("s_argv[0]=%s s_argv[1]=%s s_argv[2]=%s", s_argv[0], s_argv[1], s_argv[2]);
